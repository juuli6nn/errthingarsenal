...
function App() {
  ...
  const scrollCarousel = (direction: number): void => {
    ...
    // Sync exact floating point with real scroll using nullish coalescing for safety
    const currentScrollLeft = carouselRef.current.scrollLeft ?? 0;
    if (Math.abs(exactScrollLeft.current - currentScrollLeft) > 1) {
      exactScrollLeft.current = currentScrollLeft;
    }
